...
`Date`对象是 JavaScript 原生的时间库。Date 类型将日期保存国际标准时间（UTC）1970年1月1日00:00:00至今所经过的毫秒数。

Date 类型可以表示的时间范围：精确表示 1970 年 1 月 1 日之前及之后 285 616 年的日期（前后各1亿天，单位为毫秒）。
...
```js
Date(); // Tue Dec 01 2020 09:34:43 GMT+0800 (CST)
```

注意，即使带有参数，`Date`作为普通函数使用时，返回的还是当前时间。

```js
Date(2000, 1, 1); // Tue Dec 01 2020 09:34:43 GMT+0800 (CST)
```
...
today	//  Wed Sep 14 2016 16:50:20 GMT+0800 (CST)

// 等同于
today.toString() //  Wed Sep 14 2016 16:50:20 GMT+0800 (CST)

// 上面代码中，today是Date的实例，直接求值等同于调用toString方法。

today.toLocaleString(); // 2016/9/14 下午4:50:20
```
...
```js
// 参数为时间零点开始计算的毫秒数
new Date(1473843020000); // Wed Sep 14 2016 16:50:20 GMT+0800 (CST)

// 参数为日期字符串
new Date('2016 9 14 16:50:20');// Fri Oct 14 2016 16:50:20 GMT+0800 (CST)

// 参数为多个整数代表：年、月、日、小时、分钟、秒、毫秒
new Date(2016, 9, 14, 16, 50, 20, 0));// Fri Oct 14 2016 16:50:20 GMT+0800 (CST)
```
...
```js
new Date(2013, 15)	// Tue Apr 01 2014 00:00:00 GMT+0800 (中国标准时间)
new Date(2013, 0, 0)	// Mon Dec 31 2012 00:00:00 GMT+0800 (中国标准时间)
```
...
```js
new Date(2013, -1)	// Sat Dec 01 2012 00:00:00 GMT+0800 (中国标准时间)
new Date(2013, 0, -1)	// Sun Dec 30 2012 00:00:00 GMT+0800 (中国标准时间)
```
...
- 返回值：一个 Number，表示自 UNIX 纪元开始（1970 年 1 月 1 日 00:00:00 (UTC)）到当前时间的毫秒数。

**Date.parse(dateString)** 

- 功能：解析一个表示某个日期的字符串，并返回从1970-1-1 00:00:00 UTC 到该日期对象（该日期对象的UTC时间）的毫秒数，如果该字符串无法识别，或者一些情况下，包含了不合法的日期数值（如：2015-02-31），则返回值为NaN。
- 参数：必须，一个符合 RFC2822 或 ISO 8601 日期格式的字符串（其他格式也许也支持，但结果可能与预期不符）。
- 返回值：一个表示从1970-1-1 00:00:00 UTC到给定日期字符串所表示时间的毫秒数的数值。

> 如果参数不能解析为一个有效的日期，则返回NaN。
...
Date.parse('2016/9/14 16:50:20')
Date.parse('Sep 14 2016 16:50:20')
Date.parse('2016-9-14 16:50:20')
```
...
- 返回值：1970-1-1 00:00:00 UTC到指定的日期之间的毫秒数。
...
d.valueOf() // 1473843020000
d.getTime() // 1473843020000
```

预期为数值的场合，`Date`实例会自动调用该方法，所以可以用下面的方法计算时间的间隔。

```js
var start = new Date('2016-9-14 16:50:20');
var end = new Date('2016-10-01 00:00:00');
